...
### AI 回答「暂时还不清楚」，但知识库确实有内容

**先跑一次诊断，确认是知识库侧还是应用侧的问题：**

```bash
gh workflow run sync-dify.yml -f mode=full -f merge=true -f dry_run=false -f dataset=true
```

看日志里「自检：分别测试各种检索方式」的结果：

- **若显示召回 0 条** → 知识库侧问题，先重新同步：
  `gh workflow run sync-dify.yml -f mode=full -f merge=true`
- **若显示能召回（如混合检索 9 条、Top1 分数 0.67）** → 知识库没问题，
  问题在**应用侧**，按下面核对控制台。

#### 应用侧核对清单（Dify 控制台 → 应用「小落叶机器人」→ 编排 → 上下文）

知识库的检索设置与应用的检索设置是**两套独立配置**，改一个不一定影响另一个。
而知识库自带的「召回测试」只反映知识库侧设置，所以它通过并不代表应用侧能召回。

| 检查项 | 说明 |
|---|---|
| 关联的知识库 | 确认选的是「小落叶文档站」。若换过知识库，应用可能还指向旧的 |
| 检索方式 | ⚠️ **不要选「关键词检索」**。该知识库不生成 keywords（`--probe` 实测 automatic 与 custom 模式均为 0），选它必然召回 0 条。请用「向量检索」或「混合检索」 |
| 重排序 | 开启前确认所选重排序模型可用；模型不可用会导致检索整体失败 |
| Top K | 建议 4～6，过小则没有容错空间 |
| 分数阈值 | 确认没有开启过高的阈值。知识库实测分数约 0.67，阈值设成 0.9 会把结果全部过滤掉 |

改完点击**发布**，然后刷新网页重试（应用设置改动需要重新发布才生效）。

#### 怎么判断知识库是否真的被用上了

用你笔记里独有的词提问（模型不可能凭空知道），例如：

> LCEMonitor 是什么项目？它的仓库结构是怎样的？

若回答「暂时还不清楚」，说明知识库内容**根本没送到模型**；
若能答出具体结构，说明检索链路正常。
